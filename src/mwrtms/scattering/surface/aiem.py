...
class AIEMModel(SurfaceScattering):
    ...
    def __init__(self, wave, geometry, surface=None, *, n_max: int = 15, surface_roughness=None) -> None:
        if surface is None:
    ...
    def _compute_kirchhoff(self, R_h, R_v, polarization):
        ...
        ky = 0.0
        spectrum = self._surface.analytic_spectrum(0, kx, ky)
    ...
    def _compute_complementary(self, R_h, R_v, polarization):
        ...
        for n in range(1, min(self._n_max, 10) + 1):
            kx = -2.0 * k * np.sin(theta)
            ky = 0.0
            W_n = self._surface.analytic_spectrum(n, kx, ky)
            I_n = 0.1 * (sigma * k) ** n
